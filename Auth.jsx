    ...
    <div style={{ minHeight: '100vh', background: '#F5F4F0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 12, padding: 36, width: 380, boxShadow: '0 4px 24px rgba(0,0,0,0.10)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 800, color: '#1B4332' }}>Predmjer / Troškovnik</div>
        ...
        <div>
          ...
          {msg && (
            <div style={{
              ...
              background: msg.type === 'error' ? '#fdf0ef' : '#E8F0EC',
              color: msg.type === 'error' ? '#C0392B' : '#1B4332',
              border: `1px solid ${msg.type === 'error' ? '#f5c6c2' : '#a8d5b5'}`
            }}>{msg.text}</div>
          )}
          ...
          <div style={{ textAlign: 'center', fontSize: 13, color: '#666' }}>
            {mode === 'login' && <>
              ...
              <button onClick={() => { setMode('register'); setMsg(null); }}
                style={{ color: '#1B4332', fontWeight: 700, border: 'none', background: 'none', cursor: 'pointer', fontSize: 13 }}>
                Registrujte se
              </button>
