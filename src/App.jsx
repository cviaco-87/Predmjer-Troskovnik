              ...
              return (
                <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8 }}>
                  <select value={aktivnaPripada ? aktivnaFaza.id : ''}
                    ...
                    {fazeUFazi.length > 0 && (
                      <optgroup label="Vaše grupe radova" style={{ background: '#FBF4D6' }}>
                        {fazeUFazi.map(f => <option key={f.id} value={f.id} style={{ background: '#FBF4D6', color: '#7A5A1E' }}>{prefiks(f)}{f.naziv}</option>)}
                      </optgroup>
                    )}
                    {dostupne.length > 0 && (
            ...
            <>
              ...
              <div style={{ background: '#556575', borderRadius: 10, boxShadow: '0 1px 3px rgba(0,0,0,.15)', margin: '12px 12px 10px 12px', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
                {editFazaNazivMjesto === 'toolbar' ? (
                  ...
                  // Naslov aktivne grupe je ujedno i PREČICA za prebacivanje: umjesto odlaska
                  // u lijevi meni, grupa se mijenja odmah odavde — tu je pogled dok se radi.
                  // Dvoklik i dalje otvara preimenovanje (kao i ranije).
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                      ...
                      {aktivnaFaza?.opsti_uslovi && (
                        <button onClick={async () => { if (confirm('Obrisati opšte tehničke uslove ove grupe radova?')) { await sacuvajUslove(aktivnaFaza.id, ''); setRevizija(r => r + 1) } }}
                          style={{ background: '#FBE4E1', color: '#C0392B', border: '1px solid #E8A5A0', borderRadius: 6, padding: '6px 12px', fontSize: 11.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                          onMouseEnter={e => { e.currentTarget.style.background = '#C0392B'; e.currentTarget.style.color = '#fff' }}
                          onMouseLeave={e => { e.currentTarget.style.background = '#FBE4E1'; e.currentTarget.style.color = '#C0392B' }}>
                          🗑 Obriši
                        </button>
                      )}
                    </div>
                              ...
                              <React.Fragment key={p.id}>
                                ...
                                <tr
                                  ...
                                  <td style={{ padding: '6px 8px', textAlign: 'right', verticalAlign: 'top', borderLeft: '1px solid rgba(27,47,67,0.18)' }}>
                                    {!imadjece && <input key={`cij-${p.id}-${revizija}`} type="text" inputMode="decimal" defaultValue={p.cijena ? String(p.cijena).replace('.', ',') : ''} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.target.blur() } if (e.key === 'Escape') { e.target.value = p.cijena ? String(p.cijena).replace('.', ',') : ''; e.target.blur() } }} onBlur={e => { const v = parsiBroj(e.target.value); const unos = String(e.target.value).trim(); const dec = (unos.split(/[.,]/)[1] || '').length; e.target.value = v ? v.toFixed(Math.min(Math.max(dec, 0), 2)).replace('.', ',') : ''; azurirajPoziciju(p.id, 'cijena', v) }}
                                  ...
                                  return (
                                    <tr key={d.id}
                                      ...
                                      <td style={{ padding: '4px 8px', textAlign: 'right', borderLeft: '1px solid rgba(27,47,67,0.18)' }}>
                                        <input key={`cij-${d.id}-${revizija}`} type="text" inputMode="decimal" defaultValue={d.cijena ? String(d.cijena).replace('.', ',') : ''} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.target.blur() } if (e.key === 'Escape') { e.target.value = d.cijena ? String(d.cijena).replace('.', ',') : ''; e.target.blur() } }} onBlur={e => { const v = parsiBroj(e.target.value); const unos = String(e.target.value).trim(); const dec = (unos.split(/[.,]/)[1] || '').length; e.target.value = v ? v.toFixed(Math.min(Math.max(dec, 0), 2)).replace('.', ',') : ''; azurirajPoziciju(d.id, 'cijena', v) }}
